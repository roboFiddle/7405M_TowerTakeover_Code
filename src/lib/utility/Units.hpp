...
#ifndef INC_7405M_CODE_SRC_LIB_UTILITY_UNITS_HPP_
#define INC_7405M_CODE_SRC_LIB_UTILITY_UNITS_HPP_
...
namespace units {
  ...
  class RQuantity
  {
   ...
   public:
    ...
    constexpr RQuantity(int val) : value(static_cast<double>(val)) {}
  ...
  QUANTITY_TYPE(0, 0, -1, 0, QAngularSpeed); // Radians are unitless TODO: FUCK THIS
  QUANTITY_TYPE(0, 1, -2, 0, QAcceleration);
  QUANTITY_TYPE(0, 0, -2, 0, QAngularAcceleration);
  ...
  QUANTITY_TYPE(1, 2, -1, 0, QAngularDrag);
  ...
  constexpr RQuantity<std::ratio<0,1>, std::ratio<0,1>, std::ratio<0,1>, std::ratio<0,1>>
  operator+(const double& lhs, const RQuantity<std::ratio<0,1>, std::ratio<0,1>, std::ratio<0,1>, std::ratio<0,1>>& rhs)
  {
    return RQuantity<std::ratio<0,1>, std::ratio<0,1>, std::ratio<0,1>, std::ratio<0,1>>(lhs + rhs.getValue());
  }
  template <typename M, typename L, typename T, typename A>
  constexpr RQuantity<M, L, T, A>
  operator-(const RQuantity<M, L, T, A>& lhs, const RQuantity<M, L, T, A>& rhs)
  ...
  template <typename M, typename L, typename T, typename A>
  constexpr RQuantity<M, L, T, A>
  fabs(const RQuantity<M, L, T, A>& lhs)
  {
    return RQuantity<M, L, T, A>(std::fabs(lhs.getValue()));
  }
  ...
  constexpr QCurvature m_inv = 1 / metre;
  ...
  constexpr QTorque Nm = newton * metre;
  ...
  constexpr QForce operator"" _Nt(long double x) { return QForce(x); };
  constexpr QForce operator"" _Nt(unsigned long long int x) { return QForce(static_cast<double>(x)); };
...
#endif //INC_7405M_CODE_SRC_LIB_UTILITY_UNITS_HPP_
