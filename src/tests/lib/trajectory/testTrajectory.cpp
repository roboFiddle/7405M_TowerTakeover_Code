...
namespace test {
  void testTrajectory::test() {
    ...
    assertEquals(0.0, traj.getIndexView()->first_interpolant(), EPSILON);
    assertEquals(0.0, traj.getIndexView()->last_interpolant(), EPSILON);
    ...
    assertEquals(0.0, traj.getIndexView()->first_interpolant(), EPSILON);
    assertEquals(3.0, traj.getIndexView()->last_interpolant(), EPSILON);
